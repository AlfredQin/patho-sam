...
from torch_em.data.datasets.histopathology.puma import get_puma_paths
...
def run_interactive_segmentation(input_path, experiment_folder, model_type, split="test", start_with_box_prompt=True):
    ...
    if not (os.path.exists(data_dir) and len(os.listdir(data_dir)) > 0):
        ...
        paths = get_puma_paths(path=input_path, download=True, split=split)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_path", default="/home/qinc/Dataset/Microscopy/Histopathology/Puma/", type=str)
    parser.add_argument("-e", "--experiment_folder", default="./experiments/puma", type=str)
