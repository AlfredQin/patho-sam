...
def run_interactive_segmentation(input_path, experiment_folder, model_type, split="test", start_with_box_prompt=True):
    ...
    if not (os.path.exists(data_dir) and len(os.listdir(data_dir)) > 0):
        ...
        paths = get_tnbc_paths(path=input_path, download=True, split=split)
        ...
        for file_path in tqdm(paths):
            with h5py.File(file_path, "r") as f:
                image, label = np.array(f["raw"]).transpose(1, 2, 0), np.array(f["labels/instances"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_path", default="/home/qinc/Dataset/Microscopy/Histopathology/TNBC/", type=str)
    parser.add_argument("-e", "--experiment_folder", default="./experiments/tnbc", type=str)
