...
from torch_em.data.datasets.histopathology.srsanet import get_srsanet_paths

from scripts.utils import _pad_image


def run_interactive_segmentation(
        input_path, experiment_folder, model_type, split="test", start_with_box_prompt=True,
):
    ...
    if not (os.path.exists(data_dir) and len(os.listdir(data_dir)) > 0):
        ...
        raw_paths, label_paths = get_srsanet_paths(path=input_path, download=True, split=split)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_path", default="/home/qinc/Dataset/Microscopy/Histopathology/SRSA-Net/", type=str)
    parser.add_argument("-e", "--experiment_folder", default="./experiments/srsanet", type=str)
