...
from torch_em.data.datasets.histopathology.nuclick import get_nuclick_paths
...
def run_interactive_segmentation(
    ...
    if not (os.path.exists(data_dir) and len(os.listdir(data_dir)) > 0):
        ...
        raw_paths, label_paths = get_nuclick_paths(path=input_path, download=True, split=split)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_path", default="/home/qinc/Dataset/Microscopy/Histopathology/NuClick/", type=str)
    parser.add_argument("-e", "--experiment_folder", default="./experiments/nuclick", type=str)
