...
from torch_em.data.datasets.histopathology.nuinsseg import get_nuinsseg_paths
...
def run_interactive_segmentation(
    ...
    if not (os.path.exists(data_dir) and len(os.listdir(data_dir)) > 0):
        ...
        raw_paths, label_paths = get_nuinsseg_paths(path=input_path, download=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_path", default="/home/qinc/Dataset/Microscopy/Histopathology/NuInsSeg/", type=str)
    parser.add_argument("-e", "--experiment_folder", default="./experiments/nuinsseg", type=str)
